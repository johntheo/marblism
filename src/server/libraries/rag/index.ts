...
class Service {
  ...
  async createAndSaveFile(
    ...
    if (url === null && content === null) {
      throw new Error('URL and Content are null.')
    }
    ...
    const { vectorStore: vectorStoreGlobal } =
      await this.createVectorStoreGlobal(organizationId)
  ...
  async query(
    ...
    if (isTemporary) {
      ...
      const result = await this.createVectorStoreByTags(tags, organizationId)
      ...
      const result = await this.createVectorStoreGlobal(organizationId)
    ...
    try {
      ...
      //default to no RAG
      const modelWihtoutRag = createModel()
      const promptWithoutRAG = ChatPromptTemplate.fromMessages([
        ['system', 'You are an helpful assistant'],
        ...chat_history,
        ['user', '{input}'],
      ])

      const chainWithoutRag = promptWithoutRAG.pipe(modelWihtoutRag)

      const { content } = await chainWithoutRag.invoke({ input: prompt })
   ...
   */
  private async createVectorStoreGlobal(organizationId: string) {
   ...
   */
  private async createVectorStoreByTags(
    tags: string[],
    organizationId: string,
  ) {
